<!-- ═══════════════════════════════════════════════════════════════
     SIMPLIFIED JAVASCRIPT – Only handles UI and sends email/OTP to backend
     Backend handles all OTP generation, storage, and validation
     ═══════════════════════════════════════════════════════════════ -->
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // ── State Variables ──
        let emailVerified = false;      // Set to true only when backend confirms OTP is correct
        let resendCooldown = 0;         // Countdown in seconds (2 minutes = 120 seconds)
        let resendInterval = null;      // Timer interval reference
        ...
        emailInput.addEventListener('input', function () {
            validateEmail();
            // Reset verification if email changes after OTP was verified
            if (emailVerified) {
                emailVerified = false;
        ...
        // ════════════════════════════════════════
        // SEND OTP - Calls Backend to Generate & Send OTP
    ...
    .then(data => {
        if (data.success) {
            verifyEmailBtn.disabled = true;   // ✅ ONLY HERE
            showOTP();
        } else {
            verifyEmailBtn.disabled = false;  // ✅ retry allowed
            alert(data.message);
        }
    })
    .catch(err => {
    ...
    .finally(() => {
        ...
        // ════════════════════════════════════════
        // OTP INPUT HANDLING (4-digit OTP boxes)
        // ════════════════════════════════════════
        ...
        // ════════════════════════════════════════
        // VERIFY OTP - Sends Email & OTP to Backend for Validation
        // Backend endpoint: POST /auth/verify-otp
        // Backend handles: OTP matching, expiry check, etc.
        // ════════════════════════════════════════
        ...
        function verifyOTP() {
            ...
            // Disable OTP boxes during verification
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    clearResendTimer();
                    verifyEmailBtn.disabled = true;
                    submitBtn.disabled = false; // Enable signup button
                } else {
                    ...
                    // Shake animation and reset after delay
                    setTimeout(() => {
                        resetOTPBoxes();
                        otpBoxes[0].focus();
                    }, 900);
        ...
        // ════════════════════════════════════════
        // RESEND COOLDOWN TIMER (2 minutes = 120 seconds)
        // ════════════════════════════════════════
        ...
        // Resend Button Click - Requests new OTP from backend
        resendBtn.addEventListener('click', function () {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    startResendCooldown(120); // 2 minutes cooldown
                    otpBoxes[0].focus();
                } else {
